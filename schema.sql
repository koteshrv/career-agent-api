...
CREATE TABLE jobs (
    ...
    -- community resource that other agents may already rely on. Deleting a
    ...
    scraped_by_agent_id UUID REFERENCES agents(id) ON DELETE SET NULL,
...
CREATE INDEX idx_pulled_jobs_user ON pulled_jobs(agent_id);
...
CREATE TABLE job_reports (
    ...
    reason TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (job_id, reporter_agent_id)
);
...
-- this credit balance." admin_agent_id is ON DELETE SET NULL (not CASCADE),
-- same reasoning as jobs.scraped_by_agent_id: deleting an admin's account
-- must not erase the history of what they did.
CREATE TABLE admin_actions (
    id UUID PRIMARY KEY,
    admin_agent_id UUID REFERENCES agents(id) ON DELETE SET NULL,
